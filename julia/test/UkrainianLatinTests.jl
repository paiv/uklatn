...
@testset "DSTU_9112_A" begin

    @testset "test c2lr" begin
        data = [
            (
        ...
        for (cyr,lat) in data
            q = encode(cyr, :DSTU_9112_A)
            @test q == lat
            q = decode(lat, :DSTU_9112_A)
            @test q == cyr
            q = encode(cyr)
            @test q == lat
            q = decode(lat)
...
@testset "DSTU_9112_B" begin
    ...
    @testset "test c2lr" begin
        ...
        for (cyr,lat) in data
            q = encode(cyr, :DSTU_9112_B)
            @test q == lat
            q = decode(lat, :DSTU_9112_B)
            @test q == cyr
        end
    end
